...
main	;public static void main (String argv[]) {
	MOV done, #0;                   
	MOV vmPC, #30;                 30 instead of 0 for part 2                
	MOV vmSP, #10;                  
...
case_iadd       ;case iadd -> same as imul, but for addition
		BL popR0;             
		MOV R7, R0;             
		BL popR0;              
		ADD R0, R0, R7;              
...
case_iload	;case iload:
		BL getCode;            
		ADD vmPC, vmPC, #1;   
		BL getVars;          
		BL pushR0;           
		B while;           
		;stack[--vmSP]= vars[code[vmPC++]];
		;break;

case_istore	;case istore:
		BL getCode;          
		ADD vmPC, vmPC, #1;   
		MOV R1, R0;        
		BL popR0;            
		BL putVars;         
		B while;  
		;vars[code[vmPC++]]= stack[vmSP++];
		;break;          

case_bipush	;case bipush: 
		BL getCode;          
		ADD vmPC, vmPC, #1;   
		BL pushR0;        
		B while;           
		;stack[--vmSP]= code[vmPC++];
		;break;
...
case_if_icmplt  ;case if_icmplt -> value1 < value2 -> change vmPC to target (just like in ifgt)
		BL popR0;              
		MOV R5, R0;            
		BL popR0;              
		MOV R1, R0;          
